<section class="w-full">
    ...
    <div class="mb-6 flex flex-col gap-4 rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800 sm:flex-row sm:items-end">
        ...
        @if($this->conversionPredictionEnabled)
            <flux:button
                variant="{{ $sortByConversionScore ? 'primary' : 'ghost' }}"
                size="sm"
    ...
    <!-- AI Message Modal -->
    @if($this->aiEnabled)
        <flux:modal wire:model.self="showAiMessageModal" name="ai-message" class="w-full max-w-lg">
            <div class="space-y-6">
                <div class="flex items-center gap-2">
                    <flux:icon icon="sparkles" class="size-5 text-purple-500" />
                    <flux:heading size="lg">{{ __('AI Generated Message') }}</flux:heading>
                </div>

                @if($isGeneratingMessage)
                    <div class="flex flex-col items-center justify-center py-8">
                        <flux:icon icon="arrow-path" class="size-8 animate-spin text-purple-500" />
                        <flux:text class="mt-2 text-zinc-500 dark:text-zinc-400">{{ __('Generating message...') }}</flux:text>
                    </div>
                @elseif($generatedMessage)
                    <div class="space-y-4">
                        @if($isExistingMessage)
                            <div class="flex items-center gap-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                                <flux:icon icon="clock" class="size-4" />
                                <span>{{ __('Previously generated message found') }}</span>
                            </div>
                        @endif

                        <div class="rounded-lg bg-zinc-100 p-4 dark:bg-zinc-800">
                            <flux:text class="whitespace-pre-wrap">{{ $generatedMessage->generated_content }}</flux:text>
                        </div>

                        <div class="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
                            <flux:icon icon="cpu-chip" class="size-4" />
                            <span>{{ __('Generated by :provider', ['provider' => ucfirst($generatedMessage->ai_provider)]) }}</span>
                            @if($isExistingMessage && $generatedMessage->approved_at)
                                <span class="text-zinc-400">•</span>
                                <span>{{ $generatedMessage->approved_at->diffForHumans() }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between">
                        @if($isExistingMessage)
                            <flux:button
                                variant="ghost"
                                wire:click="regenerateAiMessage"
                                wire:loading.attr="disabled"
                                wire:target="regenerateAiMessage"
                                icon="arrow-path"
                            >
                                <span wire:loading.remove wire:target="regenerateAiMessage">{{ __('Regenerate') }}</span>
                                <span wire:loading wire:target="regenerateAiMessage">{{ __('Generating...') }}</span>
                            </flux:button>
                        @else
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-8 text-center">
                        ...
                        <flux:text class="mt-2 text-zinc-500 dark:text-zinc-400">{{ __('No message generated yet') }}</flux:text>
                    </div>
                @endif
            </div>
        </flux:modal>
