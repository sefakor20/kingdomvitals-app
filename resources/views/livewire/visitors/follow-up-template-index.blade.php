<section class="w-full">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <flux:heading size="xl" level="1">{{ __('Follow-Up Templates') }}</flux:heading>
            <flux:subheading>{{ __('Manage reusable follow-up scripts and notes for :branch', ['branch' => $branch->name]) }}</flux:subheading>
        </div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row">
        ...
        <div class="w-full sm:w-40">
            <flux:select wire:model.live="typeFilter">
                ...
                <flux:select.option value="generic">{{ __('Generic') }}</flux:select.option>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex items-center gap-2">
                                <flux:badge
                                    ...
                                    size="sm"
                                >
                                    {{ $template->type ? ucfirst($template->type->value) : __('Generic') }}
                                </flux:badge>
                                <flux:badge
    ...
    <flux:modal wire:model.self="showCreateModal" name="create-template" class="w-full max-w-lg">
        <div class="space-y-4">
            <flux:heading size="lg">{{ __('New Follow-Up Template') }}</flux:heading>

            <div class="space-y-4">
                <flux:input wire:model="name" :label="__('Template Name')" placeholder="{{ __('e.g., First Contact Call Script') }}" />
                @error('name')
                    <flux:text class="text-sm text-red-500">{{ $message }}</flux:text>
                @enderror

                <flux:select wire:model="type" :label="__('Follow-Up Type')">
                    <flux:select.option :value="null">{{ __('Generic (All Types)') }}</flux:select.option>
                    @foreach($this->followUpTypes as $followUpType)
                        <flux:select.option value="{{ $followUpType->value }}">
                            {{ ucfirst($followUpType->value) }}
                        </flux:select.option>
                    @endforeach
                </flux:select>
                <flux:text class="text-xs text-zinc-500">
                    {{ __('Generic templates can be used for any follow-up type. Type-specific templates will only appear when that type is selected.') }}
                </flux:text>
                ...
                <div x-data="{ charCount: $wire.body?.length || 0 }">
                    <flux:textarea
                        ...
                        rows="8"
                        placeholder="{{ __('Enter your follow-up script or notes template...') }}"
                    />
                    <flux:text class="mt-1 text-xs text-zinc-500">
                        <span x-text="charCount">0</span> / 1600 {{ __('characters') }}
                    </flux:text>
                    @error('body')
                        <flux:text class="text-sm text-red-500">{{ $message }}</flux:text>
                    @enderror
                    ...
                    <div class="mt-3 rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800">
                        ...
                        <flux:text class="mt-2 text-xs text-zinc-500">
                            {{ __('These placeholders will be replaced with actual visitor details when applying the template.') }}
                        </flux:text>
                    </div>
                </div>
    ...
    <!-- Toast notifications -->
    <x-toast on="template-created" type="success">
        {{ __('Template created successfully.') }}
    </x-toast>

    <x-toast on="template-updated" type="success">
        {{ __('Template updated successfully.') }}
    </x-toast>

    <x-toast on="template-deleted" type="success">
        {{ __('Template deleted successfully.') }}
    </x-toast>
</section>
