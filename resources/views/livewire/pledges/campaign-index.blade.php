<section class="w-full">
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:heading size="xl" class="mt-2">GHS {{ number_format($this->campaignStats['totalPledged'], 2) }}</flux:heading>
        </div>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                @if($campaign->goal_amount)
                                    <span class="font-medium text-zinc-900 dark:text-zinc-100">
                                        GHS {{ number_format((float) $campaign->goal_amount, 2) }}
                                    </span>
                            ...
                            <td class="px-6 py-4">
                                <div class="w-32">
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-zinc-600 dark:text-zinc-400">GHS {{ number_format($campaign->totalPledged(), 2) }}</span>
        ...
        <div class="space-y-6">
            ...
            <form wire:submit="store" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="goal_amount" type="number" step="0.01" min="0" :label="__('Goal Amount (GHS)')" />
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <div class="rounded-lg bg-zinc-50 p-3 dark:bg-zinc-800">
                        ...
                        <div class="mt-1 font-medium text-zinc-900 dark:text-zinc-100">
                            {{ $viewingCampaign->goal_amount ? 'GHS '.number_format((float)$viewingCampaign->goal_amount, 2) : '-' }}
                        </div>
                    </div>
                    <div class="rounded-lg bg-zinc-50 p-3 dark:bg-zinc-800">
                        <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Pledged') }}</div>
                        <div class="mt-1 font-medium text-zinc-900 dark:text-zinc-100">
                            GHS {{ number_format($viewingCampaign->totalPledged(), 2) }}
                        </div>
                    </div>
                    <div class="rounded-lg bg-zinc-50 p-3 dark:bg-zinc-800">
                        <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('Fulfilled') }}</div>
                        <div class="mt-1 font-medium text-zinc-900 dark:text-zinc-100">
                            GHS {{ number_format($viewingCampaign->totalFulfilled(), 2) }}
                        </div>
                    </div>
                    <div class="rounded-lg bg-zinc-50 p-3 dark:bg-zinc-800">
