<section class="w-full">
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:heading size="xl" class="mt-2">GHS {{ number_format($this->pledgeStats['totalPledged'], 2) }}</flux:heading>
        </div>
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:heading size="xl" class="mt-2">GHS {{ number_format($this->pledgeStats['totalFulfilled'], 2) }}</flux:heading>
        </div>
                        ...
                        <tr wire:key="pledge-{{ $pledge->id }}">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <span class="font-medium text-zinc-900 dark:text-zinc-100">
                                    GHS {{ number_format((float) $pledge->amount, 2) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="w-32">
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-zinc-600 dark:text-zinc-400">GHS {{ number_format((float) $pledge->amount_fulfilled, 2) }}</span>
    ...
    <flux:modal wire:model.self="showDeleteModal" name="delete-pledge" class="w-full max-w-md">
        <div class="space-y-6">
            ...
            <flux:text>
                {{ __('Are you sure you want to delete this pledge of GHS :amount? This action cannot be undone.', ['amount' => number_format((float) ($deletingPledge?->amount ?? 0), 2)]) }}
            </flux:text>
    ...
    <flux:modal wire:model.self="showPaymentModal" name="record-payment" class="w-full max-w-md">
        <div class="space-y-6">
                ...
                <div class="rounded-lg bg-zinc-50 p-4 dark:bg-zinc-800">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <div class="font-medium text-zinc-900 dark:text-zinc-100">GHS {{ number_format($recordingPaymentFor->remainingAmount(), 2) }}</div>
                        </div>
                    </div>
                    <div class="mt-3">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-zinc-600 dark:text-zinc-400">GHS {{ number_format((float) $recordingPaymentFor->amount_fulfilled, 2) }} of GHS {{ number_format((float) $recordingPaymentFor->amount, 2) }}</span>
            ...
            <form wire:submit="recordPayment" class="space-y-4">
                <flux:input wire:model="paymentAmount" type="number" step="0.01" min="0.01" :label="__('Payment Amount (GHS)')" required autofocus />
